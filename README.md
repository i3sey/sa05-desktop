...
Сокеты ядра не заворачиваются в собственный TUN: на Linux меткой `SO_MARK`
(`netbypass.Mark`, правило helper'а шлёт меченые в main-таблицу), на Windows
прямыми host-маршрутами до адресов серверов из профиля (меток там нет) —
helper ставит их до включения split-маршрутов `0.0.0.0/1 + 128.0.0.0/1`
(и `::/1 + 8000::/1`, если IPv6 не отдан системе).

## Сборка

```bash
build/build-linux.sh     # вендоринг ядра + фронтенд + все бинари в build/out (sa05, sa05ctl, sa05-helper)
go test ./...
```
...
Windows (релизный `.zip`, от администратора):

```powershell
build/fetch-wintun.sh build/out   # wintun.dll рядом с exe (в релизе уже лежит)
.\install-windows.ps1            # Program Files + служба sa05-helper
.\install-windows.ps1 -Uninstall # удаление
```

После этого TUN включается из GUI кнопкой «Установить» (UAC один раз) либо сразу
тумблером, если служба уже есть. Без службы тумблер недоступен, остальное работает.

Системный компонент `sa05-helper` — единственная часть, работающая от root/SYSTEM: он создаёт
TUN-устройство и правит маршрутизацию. GUI работает от пользователя и общается с ним через
unix-сокет с проверкой uid (Linux) или именованный канал с дескриптором безопасности (Windows).
Без установленного хелпера тумблер TUN недоступен, остальное
(ядро, системный прокси, Telegram) работает как обычно — а в самом GUI есть кнопка
«Установить», ставящая компонент с одним подтверждением прав.

На Linux helper ставится как systemd-юнит и выставляет DNS туннеля через
`resolvectl` (fallback — `/etc/resolv.conf` с бэкапом). На Windows helper ставится
как служба (`sa05-helper.exe --install-service` или `build/install-windows.ps1`
от администратора), TUN-адаптер — Wintun (`wintun.dll` рядом с exe, см.
`build/fetch-wintun.sh`), DNS вешается на адаптер, kill-switch — постоянные
null-маршруты. Локальные сети (`10/8`, `192.168/16`, …) на обеих платформах
остаются прямыми: на Linux правилом, на Windows — как более специфичные маршруты.

## Обновления

Клиент проверяет GitHub Releases этого репозитория и обновляется сам: скачивает архив
для своей платформы, проверяет подпись `ed25519` и заменяет бинарники. Ключ проверки
вшит в клиент — архив без подписи или с подписью другим ключом не устанавливается.
Приватный ключ живёт в секрете `SA05_RELEASE_KEY` и используется только релизным
workflow.

Клиент, установленный пакетным менеджером, обновляться сам не станет: каталог принадлежит
root, и запись мимо пакетного менеджера сломала бы следующее обновление — клиент об этом
скажет прямо.
